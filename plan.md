...
1. **Custom-field sources — remaining** — ACF scalar + multi-value string fields (serialized-array adapter), Save-Terms taxonomy fields (existing taxonomy path), and relationship / post_object fields (`resolve='post'` ID resolution) all land. Still to do: extend the resolve mechanism with `'user'` (user fields) and `'term'` (Save-Terms-off taxonomy) kinds, ACF date normalization, and equivalent suggestion providers for Meta Box and Pods.
