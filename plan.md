...
1. **Custom-field sources — remaining** — ACF scalar + multi-value string fields (suggestion provider + serialized-array adapter) and Save-Terms taxonomy fields (existing taxonomy path) all land. Still to do: ID → name resolution so ACF relationship / post_object / Save-Terms-off taxonomy fields show names instead of raw-ID buckets, ACF date normalization, and equivalent suggestion providers for Meta Box and Pods.
