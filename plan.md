...
1. **Custom-field sources — remaining** — ACF scalar + multi-value string fields land via the suggestion provider + serialized-array adapter (above). Still to do: ID → name resolution so ACF taxonomy / relationship / post_object fields show names instead of raw-ID buckets, ACF date normalization, and equivalent suggestion providers for Meta Box and Pods.
