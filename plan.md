...
- [x] **Meta Box source** — discovers registered Meta Box fields via `GET /integrations/metabox/suggest` (reads the `meta_box` registry). Maps the scalar/select/radio/toggle/number/date types like ACF; `taxonomy` fields map to a `kind=taxonomy` facet (Meta Box assigns terms to the post), and `post` / `user` fields ride `resolve='post'` / `resolve='user'`. **Deferred:** `taxonomy_advanced` (comma-separated term IDs in one meta value — needs a split adapter), `time`, and structural / media types.
...
1. **Custom-field sources — remaining** — the core mechanism is complete: ACF, Meta Box, and Pods all land, the resolve mechanism covers `'post'` / `'user'` / `'term'`, and ACF dates are normalized. Small deferred adapters remain: Meta Box `taxonomy_advanced` (comma-separated term IDs in a single meta value needs a split path), time-of-day fields (`time_picker` / Meta Box `time`), and Pods relationships stored in a Pods table / `wp_podsrel` rather than postmeta.
